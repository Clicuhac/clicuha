...
$stmt = $pdo->prepare('SELECT id,user_id,deleted_at FROM nicknames WHERE id=:id LIMIT 1');
$stmt->execute([':id'=>$id]);
...
$del = $pdo->prepare('UPDATE nicknames SET deleted_at=NOW() WHERE id=:id AND user_id=:uid AND deleted_at IS NULL');
