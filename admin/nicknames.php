...
if ($q !== '') {
    $where[] = '(n.title LIKE :q OR n.slug LIKE :q OR u.email LIKE :q OR u.username LIKE :q)';
    $params[':q'] = '%' . $q . '%';
}

switch ($status) {
    case 'active': $where[] = 'n.deleted_at IS NULL'; break;
    case 'free': $where[] = 'n.deleted_at IS NULL AND n.user_id IS NULL'; break;
    case 'owned': $where[] = 'n.deleted_at IS NULL AND n.user_id IS NOT NULL'; break;
    case 'anonymous': $where[] = 'n.deleted_at IS NULL AND n.is_anonymous = 1'; break;
    case 'avatar': $where[] = "n.deleted_at IS NULL AND n.avatar_path IS NOT NULL AND n.avatar_path <> ''"; break;
    case 'no_avatar': $where[] = "n.deleted_at IS NULL AND (n.avatar_path IS NULL OR n.avatar_path = '')"; break;
    case 'archived': $where[] = 'n.deleted_at IS NOT NULL'; break;
}

$sql = "SELECT n.id, n.title, n.slug, n.user_id, n.is_anonymous, n.avatar_path, n.created_at, n.deleted_at,
               u.email, u.username
        FROM nicknames n
...
$filters = [
    'all' => 'Усі', 'active' => 'Активні', 'free' => 'Вільні', 'owned' => 'З власником',
    'anonymous' => 'Анонімні', 'avatar' => 'З аватаром', 'no_avatar' => 'Без аватара', 'archived' => 'Архів'
];
?>
<!doctype html>
<html lang="<?= h($lang) ?>">
...
<div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Avatar</th><th>Clicuha</th><th>Owner</th><th>Status</th><th>Created</th><th></th></tr></thead><tbody>
<?php foreach($nicknames as $n): ?><tr>
<td><?php if(!empty($n['avatar_path'])): ?><img class="avatar" src="<?= h($n['avatar_path']) ?>" alt=""><?php else: ?><div class="avatar-empty">—</div><?php endif; ?></td>
...
<td><?= h(($n['username'] ?? '') ?: ($n['email'] ?? '—')) ?></td>
<td><?php if(!empty($n['deleted_at'])): ?><span class="badge text-bg-secondary">Архів</span><?php elseif(empty($n['user_id'])): ?><span class="badge text-bg-success">Вільна</span><?php else: ?><span class="badge text-bg-primary">Належить</span><?php endif; ?></td>
<td class="small text-secondary"><?= h((string)$n['created_at']) ?></td>
<td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="/view.php?id=<?= (int)$n['id'] ?>">View</a></td>
</tr><?php endforeach; ?>
<?php if(!$nicknames): ?><tr><td colspan="6" class="text-secondary py-4 text-center">Нічого не знайдено.</td></tr><?php endif; ?>
