...
<aside class="clic-panel"><div class="clic-panel-body"><div class="clic-panel-title">Аватар</div><div class="clic-avatar-box"><?php if($avatarUrl):?><img src="<?=h($avatarUrl)?>" alt="Аватар <?=h($clicuha['title'])?>"><?php else:?><div class="clic-avatar-placeholder"><strong><?=h(mb_strtoupper(mb_substr((string)$clicuha['title'],0,1)))?></strong><span>Поки без аватара</span></div><?php endif;?></div><div class="clic-panel-title mt-3">Статус</div><div class="clic-status-line"><span class="text-muted">Автор</span><strong><?= (int)$clicuha['is_anonymous'] === 1 ? 'Анонімно' : 'Відкрито' ?></strong></div><div class="clic-status-line"><span class="text-muted">Власність</span><strong><?= empty($clicuha['user_id']) ? 'Вільна' : 'Закріплена' ?></strong></div><?php if ($isAdmin && !empty($clicuha['user_id'])): ?><div class="clic-module-placeholder mt-3"><strong>Видно лише адміну</strong><div class="small mt-2"><span class="text-muted">Власник:</span> <?= h(($owner['username'] ?? '') ?: ($owner['email'] ?? ('user #' . (int)$clicuha['user_id']))) ?></div><?php if (!empty($owner['username']) && !empty($owner['email'])): ?><div class="small"><span class="text-muted">Email:</span> <?= h($owner['email']) ?></div><?php endif; ?><?php if ((int)$clicuha['is_anonymous'] === 1): ?><div class="small text-secondary mt-1">Для публіки автор прихований.</div><?php endif; ?></div><?php endif; ?><?php if (!empty($clicuha['created_at'])): ?><div class="clic-status-line"><span class="text-muted">Створено</span><strong><?= h(date('d.m.Y', strtotime((string)$clicuha['created_at']))) ?></strong></div><?php endif; ?><div class="clic-module-placeholder mt-3"><strong>Майбутні можливості</strong><p class="clic-panel-muted mb-0 mt-2">Події, активність, історія, купівля/передача та інші модулі.</p></div></div></aside>
