...
<div class="clic-action-row pt-2"><a href="/my_nicknames.php" class="btn btn-outline-secondary">← До моїх клікух</a><button type="submit" class="btn btn-primary">Зберегти</button></div></form>
<hr class="my-4">
<form method="post" action="/nickname_delete.php" onsubmit="return confirm('Точно відправити клікуху в архів?');"><input type="hidden" name="id" value="<?=$id?>"><input type="hidden" name="csrf_token" value="<?=h($_SESSION['csrf_token'])?>"><button type="submit" class="btn btn-sm btn-outline-danger">Відправити в архів</button></form>
</div></section>
