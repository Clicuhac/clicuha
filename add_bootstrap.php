...
function clicuha_translit(string $value): string
{
    $map = [
        ...
        'с'=>'s','т'=>'t','у'=>'u','ф'=>'f','х'=>'h','ц'=>'ts','ч'=>'ch','ш'=>'sh','щ'=>'sch','ю'=>'yu','я'=>'ya',
        'ь'=>'','ъ'=>'','’'=>'','\''=>'','ы'=>'y','э'=>'e','ё'=>'yo',
    ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '') {
        $errors[] = 'Назва Clicuha обовʼязкова.';
    }

    $slug = clicuha_slug($slugInput !== '' ? $slugInput : $title);
    if ($slug !== '') {
        $slug = clicuha_unique_slug($pdo, $slug);
    }
      ...
      <div class="card shadow-sm">
        <div class="card-body">
          ...
          <form method="post" novalidate>
            <div class="mb-3">
              <label class="form-label" for="title">Clicuha*</label>
              <input type="text" name="title" id="title" class="form-control" value="<?= h($title) ?>" required>
            </div>
            <div class="mb-3">
              <label class="form-label" for="slug">Slug (optional)</label>
              <input type="text" name="slug" id="slug" class="form-control" value="<?= h($slugInput) ?>" placeholder="lan-amazon">
            </div>
            <div class="mb-3">
              <label class="form-label" for="description">Who is… (optional)</label>
              <textarea name="description" id="description" class="form-control" rows="3"><?= h($desc) ?></textarea>
            </div>
            <div class="form-check mb-3">
              <input class="form-check-input" type="checkbox" id="is_anonymous" name="is_anonymous" value="1" <?= $isAnonymous ? 'checked' : '' ?>>
              <label class="form-check-label" for="is_anonymous">Анонімно</label>
            </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Клікуху додано</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">Нова клікуха створена і закріплена за вашим акаунтом.</div>
      <div class="modal-footer">
        <a href="/my_nicknames.php" class="btn btn-outline-secondary">Мої клікухи</a>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function makeSlug(value) {
    return value.toLowerCase().split('').map(ch => Object.prototype.hasOwnProperty.call(map, ch) ? map[ch] : ch).join('')
      .normalize('NFD').replace(/[^\w\-\s]+/g, '').replace(/[\s_]+/g, '-')
      .replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-+|-+$/g, '').slice(0, 30);
  }
  if (slug) slug.addEventListener('input', () => { slugTouched = slug.value.trim() !== ''; });
  if (title) title.addEventListener('input', () => { if (slug && !slugTouched) slug.value = makeSlug(title.value); });
  <?php if ($created): ?>
